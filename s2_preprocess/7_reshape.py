...
def del_last_model():
    for path in g.CACHE_DIR.glob(f"data_{STEP}*"):
...
for data_set_type in g.DataSetType:
    ...
    for source_zarr_path in g.iter_zarr_data_paths(STEP-1, data_set_type):
        source_zarr = zarr.open(source_zarr_path, mode="r")
        total_samples = source_zarr["label"].shape[0]

        destination_zarr = zarr.open_group(
            g.CACHE_DIR / f"data_{STEP}_{data_set_type.name}_{current_batch_index}.zarr",
            mode="w"
        )
